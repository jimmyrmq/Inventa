...
import com.djm.inventa.ui.component.EtiquetaComponent;
...
public class PanelTableProveedor  extends JPanel {
        ...
        this.tBuscar.addKeyListener(new KeyListener() {
            ...
            public void keyReleased(KeyEvent ke) {
                ...
                if(tabla.getRowCount() ==  0){
                    cb = AppContext.getInstance().getColor("TextField.backgroundError");
                    fb = Color.WHITE;
    ...
    public void updateUI() {
        super.updateUI(); // ← primero siempre

        // Guards: si los componentes no están listos todavía, salir
        if (tBuscar == null) return;

        Color bcol = null;
        if (AppContext.getInstance() != null) {
            bcol = AppContext.getInstance().getColor("back.color.dark");

            if ("LIGHT".equals(PropiedadesLookAndFeel.getPropiedad("Apariencia.lookandfeel"))) {
                bcol = AppContext.getInstance().getColor("back.color.light");
            }
        }

        if (bcol == null) bcol = Color.BLACK;

        tBuscar.setBackground(UIManager.getColor("TextField.background"));
        tBuscar.setForeground(UIManager.getColor("TextField.foreground"));

        ImageIcon icon = IconManager.get16("buscar");
        if (icon != null) {
            lupaText = new ImageIcon(ColorFilter.filterImage(icon, bcol, true));
        }
    }
}

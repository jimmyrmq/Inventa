...
public class PanelListaProducto extends JPanel implements ActionListener {
    ...
    private JPanel pVista() {
        ...
        sproducto.setSelected(AppContext.getInstance().getBoolean("PanelProducto.producto", false));
        servicio.setSelected(AppContext.getInstance().getBoolean("PanelProducto.servicio", false));
    ...
    public void updateUI(){
        Color bcol = AppContext.getInstance().getColor("back.color.dark");//new Color(190, 190, 190);
        if(PropiedadesLookAndFeel.getPropiedad("Apariencia.lookandfeel").equals("LIGTH")){
            bcol =  AppContext.getInstance().getColor("back.color.light");//new Color(90, 90, 90);
        }
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if(ae.getSource() instanceof JCheckBox) {
            ...
            if (action.equals("PRODUCTOS")) {
                AppContext.getInstance().setPropiedad("PanelProducto.producto", disp);
                CONSTANTS.CONFIG.setValue("panellistaproducto", disp);
            }
            else if (action.equals("SERVICIOS")) {
                AppContext.getInstance().setPropiedad("PanelProducto.servicio", serv);
